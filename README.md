...
| 平台 | 内容结构与写作重点 |
| --- | --- |
| [机器之心](references/styles/jiqizhixin.md) | 从研究问题切入，交代团队和核心结果，依次解释方法、数据与实验证据。 |
...
标题突出读者熟悉的模型或架构、核心动作与具体价值，兼顾简洁和吸引力；自定义方法名一般放在导语。宣传解读稿默认省略训练配置细节，保留方法机制、评测条件和关键消融。需要复现教程时可明确要求补充训练说明。

审核稿默认命名为 `<论文简称>-审核稿.docx`，不含平台名称；多平台版本放在对应子目录。修订始终以你提供的最新 Word 为底稿。
