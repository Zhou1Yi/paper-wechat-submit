...
**论文：** [Multi-Agent Forensic Reasoning for Generalizable Deepfake Video Detection](https://arxiv.org/html/2608.06865v1)  
**项目主页：** [xavierjiezou.github.io/ARGUS](https://xavierjiezou.github.io/ARGUS/)  
...
数据集的重点不只是扩大视频数量，还包括更细粒度的文本监督。作者使用多模型聚合与冲突消解流程，为视频生成视觉观察和与最终 verdict 一致的取证解释。评测协议进一步区分训练中见过的来源与未见过的身份、生成器，专门考察模型面对新型生成方法时的泛化能力。
...
**资源链接**  
论文：[arxiv.org/html/2608.06865v1](https://arxiv.org/html/2608.06865v1)  
项目与数据集：[xavierjiezou.github.io/ARGUS](https://xavierjiezou.github.io/ARGUS/)

## 作者备注（不放入正文）

- 当前依据为项目主页和 arXiv v1；作者、录用状态、实验数字及代码可用性在正式投稿前应再次核对。
- 项目页提供了框架图、数据概览和交互式案例，可作为 Word 稿件配图；下载和嵌入前需确认图片许可与清晰度。
- 本稿按机器之心 AIxiv 技术解读的“问题—资源—方法—实验—边界”顺序重写，供人工审核，不代表机器之心编辑部已确认采用。
...
我们希望投稿一篇关于 ARGUS 的论文解读，介绍其 FaceVid-Forensics-100K 数据集和多智能体取证框架。文章重点说明四个观察智能体如何从纹理、光照、运动和物理规律收集独立证据，以及裁判智能体如何在域外生成器上综合这些证据。论文和项目链接已附在正文中，稿件可根据栏目要求进一步调整。
